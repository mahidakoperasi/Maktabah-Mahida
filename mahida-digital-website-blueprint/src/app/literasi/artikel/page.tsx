    ...
    <div className="bg-cream min-h-screen">
      <section className="bg-emerald-forest py-16 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="label !text-white/60 mb-3">Literasi</p>
          <h1 className="display-md text-white">Artikel Mahida</h1>
          <p className="mt-4 max-w-2xl text-white/70">
            Tulisan, gagasan, dan catatan yang telah diterbitkan melalui Mahida Digital.
          </p>
        </div>
      </section>
            ...
            {publishedArticles.map((article) => (
              <article key={article.id} className="py-7">
                ...
                <h2 className="font-serif text-2xl font-bold text-charcoal">
                  <Link href={`/literasi/artikel/${article.slug}`} className="hover:text-emerald-forest">
                    {article.title}
                  </Link>
                </h2>
                {article.excerpt && (
                ...
                <Link
                  href={`/literasi/artikel/${article.slug}`}
