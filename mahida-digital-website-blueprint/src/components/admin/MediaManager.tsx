...
export default function MediaManager({ kind }: { kind: 'video' | 'galeri' }) {
  ...
  return <div className="grid gap-8 xl:grid-cols-2">
    <section className="space-y-3"><h1 className="font-serif text-3xl font-bold">{kind === 'video' ? 'Video YouTube' : 'Galeri Foto'}</h1><p className="text-sm text-warm-gray-600">Konten draft tidak akan terlihat oleh pengunjung.</p>{items.length === 0 && <p className="bg-white p-4">Belum ada media.</p>}{items.map((item) => <button key={item.id} className="block w-full border bg-white p-4 text-left" onClick={() => setForm({ id: item.id, title: item.title, description: item.description ?? '', url: item.youtubeId ? `https://www.youtube.com/watch?v=${item.youtubeId}` : '', imagesText: item.images?.map((image) => `${image.imageUrl}${image.caption ? ` | ${image.caption}` : ''}`).join('\n') ?? '', status: item.status })}>{item.title} <small>({item.status})</small></button>)}</section>
